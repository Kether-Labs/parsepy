...
class ParseObject:
    ...
    async def save(self, use_master_key: bool = False, session_token: str | None = None) -> None:
        ...
        if self.object_id:
            ...
            response = await client.put(path, json=payload, use_master_key=use_master_key, session_token=session_token)
        else:
            # Création (POST)
            path = f"/classes/{self.class_name}"
            response = await client.post(path, json=payload, use_master_key=use_master_key, session_token=session_token)
