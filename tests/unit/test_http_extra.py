...
async def test_http_use_master_key():
    ...
    with patch("httpx.AsyncClient.request") as mock_request:
        mock_request.return_value = MagicMock(
            status_code=200, json=lambda: {}, is_error=False
        )
...
async def test_http_network_error():
    ...
    with patch(
        "httpx.AsyncClient.request", side_effect=httpx.NetworkError("Network")
    ):  # noqa: SIM117
        with pytest.raises(ParseConnectionError):
            await client.get("/classes/GameScore")
...
def test_http_sync_retry_and_errors():
    ...
    with patch("httpx.Client.request") as mock_request:  # noqa: SIM117
